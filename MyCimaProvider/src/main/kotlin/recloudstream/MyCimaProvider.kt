...
class MyCimaProvider : MainAPI() {
    ...
    override var mainUrl = "https://wecima.watch"
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/seriestv/new/page/0" to "New Series"
    )
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val posterUrl = doc.select("wecima.separated--top").attr("data-lazy-style").getImageURL().ifEmpty { doc.select("meta[itemprop=\"thumbnailUrl\"]").attr("content") }.ifEmpty { doc.select("wecima.separated--top").attr("style").getImageURL() }
        val year = doc.select("div.Title--Content--Single-begin h1 a.unline").text().getIntFromText()
        val title = doc.select("div.Title--Content--Single-begin h1").text().replace("($year)", "").cleanTitle()
        val duration = doc.select("ul.Terms--Content--Single-begin li").firstOrNull { it.text().contains("المدة") }?.text()?.getIntFromText()
        ...
        val actors = doc.select("div.List--Teamwork > ul.Inner--List--Teamwork > li").mapNotNull {
            ...
            val image = it.attr("style").getImageURL()
            Actor(name, image)
        }
        val recommendations = doc.select("div.Grid--WecimaPosts div.GridItem").mapNotNull { element -> element.toSearchResponse() }
        ...
        return if (isMovie) {
            ...
            val seasons = doc.select("div.List--Seasons--Episodes a").not(".selected").map { it.attr("href") }
            ...
            if (moreButton.isNotEmpty()) {
                ...
                mEPS.apmap { it ->
                    if (it != null) {
                        ...
                        val document = json.output.replace("""\""", "").let { it1 -> Jsoup.parse(it1) }
    ...
    private fun Element.toSearchResponse(): SearchResponse {
        ...
        val title = select("div.Thumb--GridItem strong").text().replace(year, "").cleanTitle()

        return MovieSearchResponse(
            name = title,
            url = url.attr("href"),
            apiName = name,
            type = if(url.attr("title").contains("فيلم")) TvType.Movie else TvType.TvSeries,
            posterUrl = posterUrl,
            year = year.getIntFromText()
        )
    }
    ...
    private fun String.cleanTitle(): String {
        return this.replace("مشاهدة|فيلم|مسلسل|مترجم|انمي".toRegex(), "")
    ...
    override suspend fun loadLinks(
        ...
        doc.select(".WatchServersList > ul > li").apmap {
            val iframeUrl = it.select("btn").attr("data-url")
            val iframeName = it.select("btn").text()

            if (iframeName.contains("سيرفر ماي سيما")) return@apmap false
