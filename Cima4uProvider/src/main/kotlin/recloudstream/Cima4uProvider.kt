...
class Cima4uProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val list = doc.select("li.MovieBlock")
            ...
            .filter { val url = it.select("a").attr("href"); !url.contains("?p=") || url.substringAfter("?p=").toIntOrNull() == null }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val uniqueElements = allElements.distinctBy { it.selectFirst(".BoxTitle")?.ownText()?.cleanTitle() }
