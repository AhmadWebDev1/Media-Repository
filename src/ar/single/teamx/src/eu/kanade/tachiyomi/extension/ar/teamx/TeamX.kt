...
class TeamX : ParsedHttpSource() {
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        return SManga.create().apply {
            ...
            artist = document.select(".text-right .full-list-info:contains(الرسام) a").text().let { if (it == "غير معروف") null else it }
            author = document.select(".text-right .full-list-info:contains(المؤلف) a").text().let { if (it == "غير معروف") null else it }
            description = document.select("div.review-content").text()
            genre = document.select("div.review-author-info a").joinToString { it.text() }
            status = document.select(".text-right .full-list-info:contains(الحالة) a").text().let { if (it == "غير معروف") null else it }.parseStatus()
            thumbnail_url = document.select("div.text-right img").first()!!.absUrl("src")
        }
    }
    private fun String?.parseStatus(): Int = when {
        this == null -> SManga.UNKNOWN
        this.contains("مستمرة", ignoreCase = true) -> SManga.ONGOING
